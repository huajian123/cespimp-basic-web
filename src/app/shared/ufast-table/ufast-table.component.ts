...
export class UfastTableComponent implements OnInit, OnDestroy {
  ...
  @Input()getListHandle: Function = null;
  @Input() customRowFooterTpl: TemplateRef<any>;
  tableWidth: string;
  allChecked: boolean;
  ...
  private calcAuxData() {
    ...
    for (let i = this._tableConfig.headers.length, item; item = this._tableConfig.headers[--i];) {
      if (item.fixed) {
