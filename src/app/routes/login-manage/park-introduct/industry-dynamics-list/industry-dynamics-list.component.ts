...
export class IndustryDynamicsListComponent implements OnInit {
  ...
  goDetail(index) {
    ...
    if (index === 4) {
      this.dataInfo = `<p><span style="font-size: 16px; text-indent: 2em; color: rgb(255, 255, 255);">截止2019年10月31日，全省共有江苏华扬液碳有限责任公司等176家危险化学品生产企业（见附件）的《危险化学品生产企业安全生产许可证》不在有效期内，或者在有效期内不再从事危险化学品生产活动的，一并注销安全生产许可证，现予以公告。</span></p>`;
    }
    if (index === 5) {
